...
function getPromise(number) {
  return new Promise((fulfill, reject) => {
    if (number >= 0) fulfill('promise that will be fulfilled');
    else reject(Error('promise that will be rejected'));
  });
}
...
function queuePromises(promises) {
  ...
  let promissesChain = Promise.resolve();
  resolutionArray.length = promises.length;
  promises.forEach((promise, index) => {
    promissesChain = promissesChain.then(() => {
      return promise.then((value) => {
        resolutionArray[index] = value;
      });
    });
  });
  return promissesChain.then(() => resolutionArray.join(''));
}
